...
raw_data <- 
  readr::read_csv("data/brewmaster2_latest.csv") %>%
  filter(createdAt > new_grinder, !id %in% c("TOljBAo2Uo", "n9JRVOahqk"))
```


The data has the following fields;  
`r pander(names(raw_data))`.  

There is some notable information missing, such as the dose, origin, or the bean name.
...
```{r, echo =F}
remove_test <- which(extra_data1$roastery != "Test")
extra_data1 <- extra_data1[remove_test, ]
raw_data <- raw_data[remove_test, ]
```
...
Firstly, to make it easier the app settings should be changed so that the notes from the last brew are automatically populated for the next brew.  

**Settings > Remember Notes**

Secondly, add a JSON string in the notes section  
e.g. 
[{”batch_name”:”Sao Joao”, ”roast_date”:”2020-08-18”, ”variety”:”bourbon”, ”roast”:”medium”, ”dose”:18.2}]  

There is no limit to the extra fields added here, and you do not need to be consistent in the fields added. If a field appears in certain brews it will be NA for all others.
...
Ideally large spikes at the very beginning should be removed, as should the very end of the time series when the line is flat. Removing the flat piece at the end of the curve would give a better ending time for the shot.

```{r advanced weight over time}
my_brew_data_time <- 
  function(x, 
           split = ";", 
           pad_length = NULL, 
...
We can now use the `my_brew_data_series` to process the flow data. On top of that the flow rate can be smoothed by calculating a rolling average.
